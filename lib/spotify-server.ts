...
export async function requireUser(accessToken:string){if(!accessToken)throw new Error("UNAUTHORIZED");const sb=userClient(accessToken);const {data:{user},error}=await sb.auth.getUser(accessToken);if(error||!user)throw new Error("UNAUTHORIZED");const {data:profile}=await sb.from("profiles").select("id,is_admin").eq("id",user.id).maybeSingle();return {sb,user,profile}}
export async function requireAdmin(accessToken:string){const result=await requireUser(accessToken);if(!result.profile?.is_admin)throw new Error("FORBIDDEN");return result}

export async function refreshSpotifyToken(sb:SupabaseClient){let encrypted:string|undefined;const direct=await sb.from("jukebox_spotify_auth").select("encrypted_refresh_token").eq("id",1).maybeSingle();if(direct.data?.encrypted_refresh_token)encrypted=direct.data.encrypted_refresh_token;else{const rpc=await sb.rpc("jukebox_refresh_cipher_for_authenticated");if(typeof rpc.data==="string")encrypted=rpc.data}if(!encrypted)throw new Error("SPOTIFY_NOT_CONNECTED");const refreshToken=decrypt(encrypted);const body=new URLSearchParams({grant_type:"refresh_token",refresh_token:refreshToken});const res=await fetch("https://accounts.spotify.com/api/token",{method:"POST",headers:{Authorization:`Basic ${Buffer.from(`${clientId()}:${clientSecret()}`).toString("base64")}`,"Content-Type":"application/x-www-form-urlencoded"},body,cache:"no-store"});if(!res.ok)throw new Error("SPOTIFY_REFRESH_FAILED");return await res.json() as {access_token:string;token_type:string;expires_in:number;refresh_token?:string};}
