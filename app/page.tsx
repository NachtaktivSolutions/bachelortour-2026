...
export default function HomePage() {
  ...
  useEffect(()=>{loadPage();const channel=supabase.channel("home-v38").on("postgres_changes",{event:"*",schema:"public",table:"photos"},loadPhotos).on("postgres_changes",{event:"*",schema:"public",table:"news"},loadPage).on("postgres_changes",{event:"*",schema:"public",table:"program_items"},loadPage).on("postgres_changes",{event:"*",schema:"public",table:"hotels"},loadPage).on("postgres_changes",{event:"*",schema:"public",table:"knowledge_items"},loadPage).subscribe();const refreshOnFocus=()=>loadPage();window.addEventListener("focus",refreshOnFocus);return()=>{window.removeEventListener("focus",refreshOnFocus);supabase.removeChannel(channel);if(bachelorTimer.current)window.clearTimeout(bachelorTimer.current)}},[loadPage,loadPhotos,supabase]);
