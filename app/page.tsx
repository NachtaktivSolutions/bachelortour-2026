...
export default function HomePage() {
  ...
  const loadPhotos=useCallback(async()=>{
    const {data}=await supabase.from("photos").select("*, profiles!photos_uploader_id_fkey(name,avatar_url)").order("created_at",{ascending:false}).limit(5);
  ...
  const heroStyle=eventLoaded&&hero?{backgroundImage:`linear-gradient(180deg,rgba(0,0,0,.02),rgba(0,0,0,.95)),url("${hero}")`}:{backgroundImage:"linear-gradient(180deg,#171717,#090909)"};
  ...
  return <AuthGate><Shell>
    ...
    <SoundboardModal open={soundboardOpen} onClose={()=>setSoundboardOpen(false)}/>
  </Shell></AuthGate>;
}
