...
export default function HomePage() {
  ...
  const loadPage=useCallback(async()=>{const [eventResult,newsResult,programResult,memberResult,hotelResult,knowledgeResult]=await Promise.all([supabase.from("event_settings").select("*").eq("id",1).maybeSingle(),supabase.from("news").select("*, profiles(name,avatar_url)").order("created_at",{ascending:false}).limit(4),supabase.from("program_items").select("*").eq("is_visible",true).gte("starts_at",new Date().toISOString()).order("starts_at").limit(1).maybeSingle(),supabase.from("profiles").select("*",{count:"exact",head:true}),supabase.from("hotels").select("*").eq("is_visible",true).order("sort_order").order("created_at"),supabase.from("knowledge_items").select("*").eq("is_visible",true).order("sort_order").order("category").order("created_at")]);setEvent(eventResult.data);setEventLoaded(true);setNews((newsResult.data as NewsItem[])??[]);setNextItem(programResult.data);setMemberCount(memberResult.count??0);setHotels(hotelResult.data??[]);setKnowledge(knowledgeResult.data??[]);await loadPhotos()},[loadPhotos,supabase]);

  useEffect(()=>{loadPage();const channel=supabase.channel("home-v40").on("postgres_changes",{event:"*",schema:"public",table:"photos"},loadPhotos).on("postgres_changes",{event:"*",schema:"public",table:"news"},loadPage).on("postgres_changes",{event:"*",schema:"public",table:"program_items"},loadPage).on("postgres_changes",{event:"*",schema:"public",table:"hotels"},loadPage).on("postgres_changes",{event:"*",schema:"public",table:"knowledge_items"},loadPage).subscribe();const refreshOnFocus=()=>loadPage();window.addEventListener("focus",refreshOnFocus);return()=>{window.removeEventListener("focus",refreshOnFocus);supabase.removeChannel(channel);if(bachelorTimer.current)window.clearTimeout(bachelorTimer.current)}},[loadPage,loadPhotos,supabase]);
