...
export default function HomePage() {
  ...
  useEffect(()=>{loadPage(true);const channel=supabase.channel("home-v43").on("postgres_changes",{event:"*",schema:"public",table:"photos"},loadPhotos).on("postgres_changes",{event:"*",schema:"public",table:"news"},()=>loadPage(true)).on("postgres_changes",{event:"*",schema:"public",table:"program_items"},()=>loadPage(true)).on("postgres_changes",{event:"*",schema:"public",table:"hotels"},()=>loadPage(true)).on("postgres_changes",{event:"*",schema:"public",table:"knowledge_items"},()=>loadPage(true)).subscribe();const refreshOnFocus=()=>{if(document.visibilityState==="visible"&&Date.now()-lastLoadRef.current>30000)loadPage()};window.addEventListener("focus",refreshOnFocus);return()=>{window.removeEventListener("focus",refreshOnFocus);supabase.removeChannel(channel);if(bachelorTimer.current)window.clearTimeout(bachelorTimer.current)}},[loadPage,loadPhotos,supabase]);
