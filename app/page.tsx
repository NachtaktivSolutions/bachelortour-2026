...
export default function HomePage() {
  ...
  useEffect(()=>{
    loadPage();
    const channel=supabase.channel("home-v14")
  ...
  return <AuthGate><Shell>
    ...
    {hotels.length>0&&<section className="released-hotels"><div className="section-title"><Building2 size={20}/><h2>Unsere Hotels</h2></div><div className="hotel-strip">{hotels.map(hotel=><article className="hotel-card" key={hotel.id}><div><span className="eyebrow">UNTERKUNFT</span><h3>{hotel.name}</h3>{hotel.description&&<p>{hotel.description}</p>}<small>{hotel.address}</small></div><a className="primary-button compact-button" target="_blank" rel="noreferrer" href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(hotel.address)}`}><Navigation/>Navigation</a></article>)}</div></section>}
    ...
    {knowledge.length>0&&<section className="section knowledge-section"><div className="section-title"><Info size={20}/><h2>Wissenswertes</h2></div><div className="knowledge-grid">{knowledge.map(item=><article className="knowledge-card" key={item.id}><span className="eyebrow">{item.category.toUpperCase()}</span><h3>{item.title}</h3>{item.description&&<p>{item.description}</p>}{item.address&&<small>{item.address}</small>}<div className="knowledge-actions">{item.phone&&<a href={`tel:${item.phone}`}><Phone/>Anrufen</a>}{item.address&&<a target="_blank" rel="noreferrer" href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(item.address)}`}><Navigation/>Navigation</a>}{item.url&&<a target="_blank" rel="noreferrer" href={item.url}><ExternalLink/>Öffnen</a>}</div></article>)}</div></section>}
