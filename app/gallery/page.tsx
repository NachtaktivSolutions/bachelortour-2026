    ...
    try {
      for (let index = 0; index < files.length; index += 1) {
        ...
        try {
          ...
          const file = await optimizeImage(original, message => setUploadProgress(`Foto ${index + 1} von ${files.length}: ${message}`));
          setUploadProgress(`Foto ${index + 1} von ${files.length} wird hochgeladen …`);
          const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, "-");
          const path = `${profile.id}/${Date.now()}-${index}-${safeName}`;
          const { error: uploadError } = await supabase.storage.from("photos").upload(path, file, { upsert: false, cacheControl: "31536000", contentType: "image/jpeg" });
          if (uploadError) throw uploadError;
          const url = supabase.storage.from("photos").getPublicUrl(path).data.publicUrl;
          const { error: insertError } = await supabase.from("photos").insert({ image_url: url, uploader_id: profile.id });
          if (insertError) throw insertError;
        } catch (uploadError) {
          failures.push(`${original.name}: ${uploadError instanceof Error ? uploadError.message : "Fehler"}`);
        }
      }
      await load();
      if (failures.length) setError(`${files.length - failures.length} von ${files.length} Fotos hochgeladen. ${failures.join(" | ")}`);
    } finally {
      setBusy(false); setUploadProgress(""); e.target.value = "";
    }
  }
