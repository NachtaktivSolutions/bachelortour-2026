...
export default function GalleryPage() {
  ...
  useEffect(() => {
    load();
    const channel = supabase.channel("gallery-v8")
  ...
  async function upload(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      for (let index = 0; index < files.length; index += 1) {
        ...
        setUploadProgress(`Foto ${index + 1} von ${files.length} wird hochgeladen …`);

        try {
          const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, "-");
          const path = `${profile.id}/${Date.now()}-${index}-${safeName}`;
          ...
          failures.push(`${file.name}: ${uploadError instanceof Error ? uploadError.message : "Fehler"}`);
        }
      }

      await load();
      if (failures.length) setError(`${files.length - failures.length} von ${files.length} Fotos hochgeladen. ${failures.join(" | ")}`);
    } finally {
      setBusy(false);
      setUploadProgress("");
      e.target.value = "";
  ...
  async function comment(e: FormEvent<HTMLFormElement>) {
    ...
    if (!body) return;
    const { error: commentError } = await supabase.from("photo_comments").insert({
  ...
  return <AuthGate><Shell>
    <div className="page-heading">
      <span className="eyebrow">KEINE BEWEISE, KEIN VERBRECHEN</span>
      <h1>Galerie</h1>
      <p>Mehrere Fotos auswählen, liken, kommentieren und im Vollbild ansehen.</p>
    </div>

    <div className="gallery-toolbar">
      <label className="upload-fab">
        <Camera />
        {busy ? uploadProgress || "Lädt …" : "Fotos hochladen"}
        <input type="file" accept="image/*" multiple onChange={upload} disabled={busy} />
      </label>
      <button className="secondary-button" onClick={load} disabled={busy}><RefreshCw />Aktualisieren</button>
    </div>
    ...
    {loading ? <div className="empty-card">Galerie wird geladen …</div> : !photos.length ? <div className="empty-card">Noch keine Fotos – lade das erste hoch.</div> :
      <div className="photo-grid">{photos.map(photo => {
        ...
        return <figure key={photo.id}>
          ...
          <figcaption>
            <span>{photo.profiles?.name || "Mitglied"}<small>{new Date(photo.created_at).toLocaleString("de-DE")}</small></span>
            <div>
              <button className={liked ? "liked" : ""} onClick={() => toggleLike(photo)}><Heart fill={liked ? "currentColor" : "none"} />{photo.photo_likes?.length || 0}</button>
              <button onClick={() => setSelectedId(photo.id)}><MessageCircle />{photo.photo_comments?.length || 0}</button>
            </div>
          </figcaption>
        </figure>;
      })}</div>}

    {selected && <div className="photo-modal" onClick={() => setSelectedId(null)}>
