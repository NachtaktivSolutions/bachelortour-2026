...
async function getContext(req:NextRequest){
  ...
  const {data:profile}=await admin.from("profiles").select("id,name").eq("id",user.id).single();
...
export async function GET(req:NextRequest){
  try{
    ...
    const [{data:orders,error},{data:stamp}]=await Promise.all([
    ...
    return NextResponse.json({orders:orders??[],isNewBachelor:Boolean(stamp)});
...
export async function POST(req:NextRequest){
  try{
    ...
    if(action==="claim"){
      ...
      if(error)throw error;if(!updated)return NextResponse.json({error:"Da war jemand schneller."},{status:409});
      await sendPush(ctx.admin,[order.requester_id],{title:"🍺 Bachelorando ist unterwegs",body:`${ctx.profile.name} übernimmt deine Bestellung: ${order.quantity}× ${order.item}.`,url:"/bachelorando",tag:`bachelorando-claimed-${orderId}`});
