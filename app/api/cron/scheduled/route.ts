...
export async function POST(req: NextRequest) {
  ...
  if (!supplied || !config?.token || supplied !== config.token) return NextResponse.json({ error: "Nicht autorisiert." }, { status: 401 });

  const { data: jobs, error } = await admin.from("scheduled_jobs").select("*").eq("status", "pending").lte("scheduled_for", new Date().toISOString()).order("scheduled_for").limit(25);
  ...
  for (const job of jobs ?? []) {
    ...
    try {
      if (job.job_type === "news") {
        ...
        const { error: insertError } = await admin.from("news").insert(payload);
        if (insertError) throw insertError;
        if (sendPushAfter) await sendPush(admin, payload.title, payload.body, "/");
...
async function sendPush(admin: any, title: string, body: string, targetUrl: string) {
