...
export async function POST(request:NextRequest){
  try{
    ...
    let res:Response;
    if(body.action==="queue"&&body.uri){const qs=body.deviceId?`&device_id=${encodeURIComponent(body.deviceId)}`:"";res=await spotifyFetch(sb,`/me/player/queue?uri=${encodeURIComponent(body.uri)}${qs}`,{method:"POST"});}
    ...
    else if(body.action==="transfer"&&body.deviceId){res=await spotifyFetch(sb,"/me/player",{method:"PUT",body:JSON.stringify({device_ids:[body.deviceId],play:false})});}
