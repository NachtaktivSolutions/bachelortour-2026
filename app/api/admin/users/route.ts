...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (typeof temporaryPassword === "string" && temporaryPassword.length >= 6) {
      const { error } = await auth.admin.auth.admin.updateUserById(userId, { password: temporaryPassword });
      if (error) throw error;
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!userId) return NextResponse.json({ error: "Benutzer-ID fehlt." }, { status: 400 });
    if (userId === auth.user.id) return NextResponse.json({ error: "Du kannst dich nicht selbst löschen." }, { status: 400 });
