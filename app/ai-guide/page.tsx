...
type Action={type:string;label:string;subtitle?:string;appUrl?:string;navigationUrl?:string;latitude?:number;longitude?:number;address?:string|null;phone?:string|null;website?:string|null};
type Location={latitude:number;longitude:number};

const starters=["Was steht als Nächstes im freigegebenen Programm?","Wo ist hier eine gute Bar?","Wie komme ich zu unserem sichtbaren Hotel?","Welche Informationen sind aktuell freigeschaltet?"];

export default function AiGuidePage(){
  const supabase=useMemo(()=>createClient(),[]);
  const [messages,setMessages]=useState<Message[]>([{role:"assistant",content:"Servus! Ich bin der Firestarter KI-Guide. Ich kenne ausschließlich bereits freigeschaltete Tourinformationen. Geheime Programmpunkte, Ziele und Teilnehmerstandorte kann ich weder sehen noch verraten."}]);
  ...
  async function loadStatus(){try{const headers=await authHeader();const response=await fetch("/api/ai-guide",{headers});const data=await response.json();if(!response.ok)throw new Error(data.error||"Status konnte nicht geladen werden.");setConfigured(Boolean(data.configured));setPlacesConfigured(Boolean(data.placesConfigured));setRemaining(data.remaining??null)}catch(e){setError(e instanceof Error?e.message:"Status konnte nicht geladen werden.")}}

  function useLocation(){
    if(!navigator.geolocation){setLocationStatus("Standort wird von diesem Gerät nicht unterstützt.");return}
    setLocationStatus("Standort wird ermittelt …");
    navigator.geolocation.getCurrentPosition(pos=>{setLocation({latitude:pos.coords.latitude,longitude:pos.coords.longitude});setLocationStatus(`Standort aktiv · Genauigkeit ca. ${Math.round(pos.coords.accuracy)} m`)},err=>{setLocation(null);setLocationStatus(err.code===1?"Standort wurde nicht freigegeben.":"Standort konnte nicht ermittelt werden.")},{enableHighAccuracy:true,timeout:12000,maximumAge:60000});
  }
  ...
  async function send(value?:string){
    ...
    try{
      ...
      if(!response.ok)throw new Error(data.error||"Der KI-Guide konnte nicht antworten.");
      setMessages(current=>[...current,{role:"assistant",content:data.answer,actions:data.actions||[]}]);
      if(data.remaining!=null)setRemaining(data.remaining);
    }catch(e){setError(e instanceof Error?e.message:"Der KI-Guide konnte nicht antworten.")}finally{setBusy(false)}
  }
    ...
    <div style={{display:"grid",gap:14,minHeight:"calc(100vh - 140px)"}}>
      ...
      <section style={{display:"flex",flexDirection:"column",gap:12,minHeight:320,maxHeight:"calc(100vh - 430px)",overflowY:"auto",padding:"4px 2px 12px"}}>
        {messages.map((message,index)=><article key={index} style={{alignSelf:message.role==="user"?"flex-end":"flex-start",width:"min(92%,680px)",padding:"13px 14px",borderRadius:message.role==="user"?"18px 18px 5px 18px":"18px 18px 18px 5px",background:message.role==="user"?"linear-gradient(135deg,#ff6a00,#d94d00)":"#1b1b1b",border:message.role==="user"?"0":"1px solid #2d2d2d"}}>
          ...
          {!!message.actions?.length&&<div style={{display:"grid",gap:8,marginTop:12}}>{message.actions.map((action,i)=><ActionCard key={`${action.label}-${i}`} action={action}/>)}</div>}
        </article>)}
