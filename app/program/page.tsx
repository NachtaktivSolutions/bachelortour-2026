...
export default function ProgramPage(){
  const [items,setItems]=useState<ProgramItem[]>([]);const supabase=useMemo(()=>createClient(),[]);
  ...
  useEffect(()=>{
    ...
    window.setTimeout(()=>document.getElementById(id)?.scrollIntoView({behavior:"smooth",block:"center"}),120);
  },[items]);
  ...
  return <AuthGate><Shell><div className="page-heading"><span className="eyebrow">DER PLAN</span><h1>Programm</h1><p>Alle bereits bekanntgegebenen Zeiten und Treffpunkte auf einen Blick.</p></div>
    {Object.entries(groups).map(([day,dayItems])=><section className="program-day" key={day}><h2><CalendarDays/>{day}</h2>{dayItems.map(item=><article className="program-item" id={`program-${item.id}`} style={{scrollMarginTop:"140px"}} key={item.id}>
