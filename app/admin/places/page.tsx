...
import { Building2, Eye, EyeOff, Info, MapPinned, Navigation, Pencil, Plus, Trash2 } from "lucide-react";
import { AuthGate } from "@/components/auth-gate";
import { Shell } from "@/components/shell";
import { createClient } from "@/lib/supabase/client";
import { useApp } from "@/components/app-provider";
import { berlinLocalToIso } from "@/lib/datetime";
...
export default function PlacesAdminPage(){
  ...
  async function addProgram(e:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      const {error}=await supabase.from("program_items").insert({
        title:String(f.get("title")).trim(), description:String(f.get("description")).trim()||null,
        address, latitude:coords.latitude, longitude:coords.longitude,
        starts_at:berlinLocalToIso(String(f.get("starts_at"))), ends_at:berlinLocalToIso(String(f.get("ends_at"))),
        created_by:session!.user.id
      });
      if(error)throw error; form.reset(); setStatus(`Programmpunkt gespeichert: ${coords.display_name}`); await load();
  ...
  async function addKnowledge(e:FormEvent<HTMLFormElement>){
    e.preventDefault(); const form=e.currentTarget; const f=new FormData(form); setBusy(true); setStatus("");
    try{
      const address=String(f.get("address")).trim(); let coords:{latitude:number;longitude:number}|null=null;
      if(address)coords=await geocode(address);
  ...
  async function toggle(table:"hotels"|"knowledge_items",id:string,current:boolean){
    const {error}=await supabase.from(table).update({is_visible:!current,updated_at:new Date().toISOString()}).eq("id",id);
    setStatus(error?error.message:(!current?"Inhalt ist jetzt sichtbar.":"Inhalt wurde verborgen.")); await load();
  }
  async function remove(table:"hotels"|"knowledge_items",id:string,label:string){
    if(!confirm(`${label} wirklich löschen?`))return;
    const {error}=await supabase.from(table).delete().eq("id",id); setStatus(error?error.message:`${label} gelöscht.`); await load();
  }

  return <AuthGate admin><Shell>
    <div className="page-heading"><span className="eyebrow">GEHEIME ORTE</span><h1>Hotels & Wissenswertes</h1><p>Inhalte vorbereiten und erst dann für alle Bachelor sichtbar schalten.</p></div>
    {status&&<div className="status">{status}</div>}
    <div className="admin-grid secret-admin-grid">
      <form className="admin-card" onSubmit={addProgram}><MapPinned/><h2>Programmpunkt mit Adresse</h2><input name="title" placeholder="Titel" required/><textarea name="description" placeholder="Beschreibung"/><input name="address" placeholder="Vollständige Adresse" required/><label>Beginn<input name="starts_at" type="datetime-local" required/></label><label>Ende<input name="ends_at" type="datetime-local" required/></label><small>Breiten- und Längengrad werden automatisch aus der Adresse ermittelt.</small><button className="primary-button" disabled={busy}><Plus/>Programmpunkt speichern</button></form>
      ...
      <section className="admin-card admin-wide"><h2><Building2/> Hotels verwalten</h2>{hotels.map(item=><div className="admin-content-row" key={item.id}><div><strong>{item.name}</strong><small>{item.address}</small></div><div className="admin-row-actions"><button className="secondary-button" onClick={()=>toggle("hotels",item.id,item.is_visible)}>{item.is_visible?<><EyeOff/>Verbergen</>:<><Eye/>Sichtbar schalten</>}</button><a className="secondary-button" target="_blank" href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(item.address)}`}><Navigation/>Navigation</a><button className="danger-button" onClick={()=>remove("hotels",item.id,"Hotel")}><Trash2/>Löschen</button></div></div>)}{!hotels.length&&<p>Noch keine Hotels angelegt.</p>}</section>
