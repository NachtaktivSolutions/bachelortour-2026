...
export default function AdminSoundsPage(){
  ...
  async function createSound(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!profile?.is_admin||!file)return;setBusy(true);setStatus("");const form=new FormData(event.currentTarget);try{const extension=file.name.split(".").pop()?.toLowerCase()||"mp3";const safe=`${Date.now()}-${crypto.randomUUID()}.${extension}`;const path=`sounds/${safe}`;const upload=await supabase.storage.from("tour-sounds").upload(path,file,{contentType:file.type||"audio/mpeg",cacheControl:"3600"});if(upload.error)throw upload.error;const nextOrder=sounds.length?Math.max(...sounds.map(sound=>sound.sort_order))+1:0;const insert=await supabase.from("tour_sounds").insert({title:String(form.get("title")).trim(),category:String(form.get("category")).trim()||"Firestarter 2026",description:String(form.get("description")).trim()||null,storage_path:path,duration_seconds:duration,is_visible:Boolean(form.get("is_visible")),sort_order:nextOrder,created_by:profile.id});if(insert.error){await supabase.storage.from("tour-sounds").remove([path]);throw insert.error}event.currentTarget.reset();setFile(null);setDuration(null);setStatus("Sound gespeichert. Du kannst ihn unten bearbeiten und verschieben.");await load()}catch(error){setStatus(error instanceof Error?error.message:"Sound konnte nicht gespeichert werden.")}finally{setBusy(false)}}
  ...
  function moveSound(targetId:string){
    const sourceId=draggedIdRef.current;if(!sourceId||sourceId===targetId)return;
    setSounds(current=>{
      const from=current.findIndex(sound=>sound.id===sourceId);const to=current.findIndex(sound=>sound.id===targetId);if(from<0||to<0)return current;
      const next=[...current];const [moved]=next.splice(from,1);next.splice(to,0,moved);
      const ordered=next.map((sound,index)=>({...sound,sort_order:index}));soundsRef.current=ordered;return ordered;
    });
  }

  async function saveSoundOrder(){
    const ordered=soundsRef.current;
    const updates=await Promise.all(ordered.map(sound=>supabase.from("tour_sounds").update({sort_order:sound.sort_order,updated_at:new Date().toISOString()}).eq("id",sound.id)));
