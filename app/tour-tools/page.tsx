...
const statuses=["freudig","beim Saufen","total breit","beim Tanzen","beim Schlafen","brauche Hilfe!"];
...
export default function TourToolsPage(){
  ...
  useEffect(()=>{
    load();
    const channel=supabase.channel("tour-tools-live-v2")
  ...
  const activeEvent=events.find(event=>event.is_open&&(!event.starts_at||new Date(event.starts_at).getTime()<=now)&&(!event.closes_at||new Date(event.closes_at).getTime()>now))??null;
  ...
  async function sendHelp(){
    ...
    try{
      ...
      const {error:updateError}=await supabase.from("profiles").update({latitude:position.coords.latitude,longitude:position.coords.longitude,location_updated_at:nowIso,share_location:true,participant_status:"brauche Hilfe!",status_updated_at:nowIso}).eq("id",profile.id);
