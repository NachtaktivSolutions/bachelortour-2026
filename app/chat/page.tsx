...
export default function ChatPage() {
  ...
  useEffect(()=>{load();const channel=supabase.channel("chat").on("postgres_changes",{event:"INSERT",schema:"public",table:"chat_messages"},load).subscribe();return()=>{supabase.removeChannel(channel)}},[]);
  useEffect(()=>end.current?.scrollIntoView({behavior:"smooth"}),[messages]);

  async function send(e:FormEvent<HTMLFormElement>){e.preventDefault();const body=text.trim();if(!body||!profile)return;await supabase.from("chat_messages").insert({body,sender_id:profile.id});setText("");setEmojiOpen(false)}
  async function sendImage(e:ChangeEvent<HTMLInputElement>){const file=e.target.files?.[0];if(!file||!profile)return;setUploading(true);const path=`${profile.id}/${Date.now()}-${file.name.replace(/\s+/g,"-")}`;const up=await supabase.storage.from("photos").upload(path,file);if(!up.error){const url=supabase.storage.from("photos").getPublicUrl(path).data.publicUrl;await supabase.from("chat_messages").insert({body:"📷 Foto",image_url:url,sender_id:profile.id})}setUploading(false);e.target.value=""}
