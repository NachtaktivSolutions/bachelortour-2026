...
export function PushBootstrap() {
  ...
  useEffect(()=>{
    ...
    if(Notification.permission==="granted"&&supportsPush()){
      ensurePushSubscription(session.user.id).catch(console.error);
    }

    // Auf Mobilgeräten werden Standort und Push erst in der installierten App abgefragt.
    if(mobile&&!installed)return;
    if(localStorage.getItem(setupKey)==="done")return;

    let next:SetupStep="done";
    ...
    if(next==="done"){localStorage.setItem(setupKey,"done");return}
    setStep(next);
  ...
  async function enableLocation(){
    ...
    try{
      ...
      if(err instanceof GeolocationPositionError){
        if(err.code===err.PERMISSION_DENIED)setError("Standort wurde blockiert. Bitte bei der Browser-Abfrage auf „Zulassen“ tippen.");
        else if(err.code===err.TIMEOUT)setError("Die Standortabfrage hat zu lange gedauert. Bitte noch einmal versuchen.");
  ...
  async function activatePush(){
    ...
    try{
      ...
      setError(message);
      if(Notification.permission==="denied")setStep("blocked");
    }finally{setBusy(false)}
  }
  ...
  return <div className="push-consent-backdrop setup-wizard-backdrop" role="dialog" aria-modal="true" aria-labelledby="device-setup-title">
    <section className="push-consent-card setup-wizard-card">
      <button className="push-consent-close" onClick={finishSetup} aria-label="Schließen"><X/></button>
      <div className="setup-progress"><span>{number}</span><i className={step!=="location"?"complete":""}/><i className={step==="push"||step==="blocked"||step==="unsupported"?"active":""}/></div>

      {step==="location"&&<>
        <div className="push-consent-icon"><LocateFixed/></div><span className="eyebrow">SCHRITT 1</span>
        <h2 id="device-setup-title">Standort einschalten</h2>
        <p>Damit die anderen dich auf der Live-Karte sehen können. Tippe danach bei der Geräte-Abfrage auf <strong>„Zulassen“</strong>.</p>
        {error&&<div className="error">{error}</div>}
        <button className="primary-button setup-main-button" onClick={enableLocation} disabled={busy}><MapPin/>{busy?"Standort wird geprüft …":"Standort erlauben"}</button>
        <button className="push-later-button" onClick={continueWithoutLocation}>Ohne Standort weiter</button>
      </>}

      {step==="push"&&<>
        <div className="push-consent-icon"><BellRing/></div><span className="eyebrow">SCHRITT 2</span>
        <h2 id="device-setup-title">Nachrichten erlauben</h2>
        <p>So bekommst du Treffpunkte und wichtige Änderungen. Tippe danach bei der Geräte-Abfrage auf <strong>„Zulassen“</strong>.</p>
        {error&&<div className="error">{error}</div>}
        <button className="primary-button setup-main-button" onClick={activatePush} disabled={busy}><BellRing/>{busy?"Wird aktiviert …":"Push erlauben"}</button>
        <button className="push-later-button" onClick={finishSetup}>Später machen</button>
      </>}

      {step==="blocked"&&<>
        <div className="push-consent-icon warning"><BellOff/></div><span className="eyebrow">BENACHRICHTIGUNGEN BLOCKIERT</span>
        <h2 id="device-setup-title">Push wieder freigeben</h2>
        <p>Öffne die App- oder Website-Einstellungen deines Handys und stelle <strong>Benachrichtigungen auf „Zulassen“</strong>. Öffne danach Firestarter erneut.</p>
        <div className="setup-browser-hint">Android: App-Symbol gedrückt halten → App-Info → Benachrichtigungen<br/>iPhone: Einstellungen → Mitteilungen → Firestarter 2026</div>
        <button className="primary-button setup-main-button" onClick={()=>window.location.reload()}>Erneut prüfen</button>
        <button className="push-later-button" onClick={finishSetup}>Später machen</button>
      </>}

      {step==="unsupported"&&<>
        <div className="push-consent-icon warning"><BellOff/></div><span className="eyebrow">FAST FERTIG</span>
        <h2 id="device-setup-title">Push hier nicht verfügbar</h2>
        <p>Die App funktioniert trotzdem vollständig. Für Benachrichtigungen öffne und installiere sie auf Android am besten mit Chrome oder Samsung Internet, auf dem iPhone mit Safari.</p>
        <button className="primary-button setup-main-button" onClick={finishSetup}><Check/>App verwenden</button>
      </>}
    </section>
  </div>;
}
...
async function enablePush(userId: string) {
  if (!("Notification" in window) || !("serviceWorker" in navigator) || !("PushManager" in window)) throw new Error("Dieser Browser unterstützt Push nicht.");
  if (!process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY) throw new Error("Push ist serverseitig noch nicht vollständig eingerichtet.");
  const permission = await Notification.requestPermission();
  if (permission === "denied") throw new Error("Benachrichtigungen wurden blockiert.");
...
async function saveSubscription(userId: string, subscription: PushSubscription) {
  const supabase = createClient();
  const payload={user_id:userId,endpoint:subscription.endpoint,subscription:subscription.toJSON()};
  const { error } = await supabase.from("push_subscriptions").upsert(payload,{onConflict:"endpoint"});
  if (error) throw new Error(`Registrierung konnte nicht gespeichert werden: ${error.message}`);
}
...
function pushStateText(state: PushState) {
  ...
  if (state === "blocked") return "Im Browser blockiert. Bitte über die App- oder Website-Einstellungen freigeben.";
