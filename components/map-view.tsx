...
function eventMarker(type:"program"|"meeting"){
  if(type==="meeting") return L.divIcon({className:"event-pin-v29 meeting",html:'<div class="event-pin-shell"><span class="meeting-flame">🔥</span><b>Treffpunkt</b></div>',iconSize:[118,106],iconAnchor:[59,92],popupAnchor:[0,-85]});
  return L.divIcon({className:"event-pin-v29 program",html:'<div class="event-pin-shell"><span class="program-star">★</span><b>Programmpunkt</b></div>',iconSize:[130,102],iconAnchor:[65,88],popupAnchor:[0,-82]});
}

function FitMap({points}:{points:[number,number][]}){const map=useMap();const key=useMemo(()=>JSON.stringify(points),[points]);useEffect(()=>{const timer=window.setTimeout(()=>{map.invalidateSize();if(!points.length)return;if(points.length===1)map.setView(points[0],15);else map.fitBounds(L.latLngBounds(points),{paddingTopLeft:[80,80],paddingBottomRight:[80,150],maxZoom:15,animate:true})},150);return()=>window.clearTimeout(timer)},[map,key]);return null}
