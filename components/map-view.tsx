...
export function MapView({pins,programItems,members,fitRequest}:Props){
  ...
  async function search(e:FormEvent){
    e.preventDefault();const term=query.trim();if(!term||searching)return;setSelected(null);setSelectedTarget(null);
    const localCenter=mapCenter;const cacheKey=searchCacheKey(term,localCenter[0],localCenter[1]);const cached=readSearchCache(cacheKey);setSearchError("");
...
function MapDetailSheet({selected,onClose}:{selected:Exclude<Selected,null>;onClose:()=>void}){
  let content:React.ReactNode;let nav="";
  if(selected.kind==="program"){const i=selected.item,m=i.marker_type??"program";nav=navigationUrl(i.latitude!,i.longitude!,i.address);content=<><span className={`event-popup-kind ${m}`}>{m==="meeting"?"Treffpunkt":"Programmpunkt"}</span><strong>{i.title}</strong>{i.starts_at&&<small>{formatBerlin(i.starts_at)}</small>}{i.description&&<p>{i.description}</p>}{i.address&&<span>{i.address}</span>}</>}
  else if(selected.kind==="pin"){const i=selected.item;nav=navigationUrl(i.latitude,i.longitude,i.address);content=<><span className="event-popup-kind meeting">Treffpunkt</span><strong>{i.title}</strong>{i.description&&<p>{i.description}</p>}{i.address&&<span>{i.address}</span>}</>}
  else if(selected.kind==="poi"){const i=selected.item;nav=navigationUrl(i.latitude,i.longitude,i.address);content=<><span>{categoryEmoji(i.category)} {categoryLabel(i.category)}</span><strong>{i.name}</strong>{i.address&&<small>{i.address}</small>}</>}
  else{const m=selected.item,s=m.participant_status||"kein Status";nav=navigationUrl(m.latitude!,m.longitude!);content=<><div className="member-popup-head"><span className="member-popup-avatar">{m.avatar_url?<img src={m.avatar_url} alt=""/>:<b>{initials(m.name)}</b>}</span><div><strong>{selected.own?"Mein Standort":m.name}</strong><span className={`map-status-badge ${statusClass(s)}`}>{statusDisplay(s)}</span></div></div><small>{m.location_updated_at?relativeUpdated(m.location_updated_at,selected.stale):"Standort geteilt"}</small>{selected.stale&&<p>Dieser Standort wurde seit mehr als 30 Minuten nicht aktualisiert.</p>}{!selected.own&&m.phone&&<a href={`tel:${m.phone}`}>Anrufen</a>}</>}
  return <section aria-label="Kartendetails" style={{position:"absolute",left:10,right:10,bottom:10,zIndex:900,maxHeight:"45%",overflowY:"auto",padding:"18px",borderRadius:"22px",background:"rgba(12,12,12,.97)",boxShadow:"0 12px 40px rgba(0,0,0,.55)",border:"1px solid rgba(255,255,255,.12)",pointerEvents:"auto",touchAction:"pan-y"}}><button type="button" aria-label="Details schließen" onClick={onClose} style={{position:"absolute",right:10,top:10,width:38,height:38,borderRadius:"50%",border:"1px solid rgba(255,255,255,.16)",background:"#202020",color:"white",fontSize:24,lineHeight:1}}>×</button><div className="map-popup" style={{paddingRight:38}}>{content}<a target="_blank" rel="noreferrer" href={nav} onClick={onClose}>Dorthin navigieren</a></div></section>
}
...
function MapController({points,fitRequest,layers,setPois,searchTarget,selectedTarget,onCenterChange,onMapClick}:{points:[number,number][];fitRequest:number;layers:string[];setPois:(v:Poi[])=>void;searchTarget:[number,number]|null;selectedTarget:[number,number]|null;onCenterChange:(v:[number,number])=>void;onMapClick:()=>void}){
  const map=useMap();const initialized=useRef(false);const lastFit=useRef(fitRequest);const loadTimer=useRef<number|null>(null);const abortRef=useRef<AbortController|null>(null);const lastLoad=useRef<{lat:number;lon:number;zoom:number;layers:string}|undefined>();
  ...
  useEffect(()=>{const c=map.getCenter();onCenterChange([c.lat,c.lng]);requestPois(c.lat,c.lng,true);return()=>{if(loadTimer.current)window.clearTimeout(loadTimer.current);abortRef.current?.abort()}},[layers]);return null
}
