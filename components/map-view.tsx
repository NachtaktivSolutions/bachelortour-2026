...
function memberIcon(member:Profile,own:boolean,stale:boolean){const initials=member.name.split(/\s+/).filter(Boolean).slice(0,2).map(part=>part[0]?.toUpperCase()).join("")||"?";const content=member.avatar_url?`<img src="${escapeHtml(member.avatar_url)}" alt=""/>`:`<span>${escapeHtml(initials)}</span>`;const status=member.participant_status||"kein Status";return L.divIcon({className:`map-marker profile-map-marker${own?" own":""}${stale?" stale":""} ${statusClass(status)}`,html:`<div class="profile-pin-photo" style="${stale?"filter:grayscale(1);opacity:.58;":""}">${content}</div><b class="profile-pin-status">${escapeHtml(status)}</b>`,iconSize:[96,76],iconAnchor:[48,60]})}
