...
export function MapView({pins,programItems,members,fitRequest}:Props){
  ...
  return <div className="smart-map-wrap">
    <MapContainer center={center} zoom={14} className="map-container">
      ...
      <MapController points={points} fitRequest={fitRequest} layers={layers} setPois={setPois} searchTarget={searchTarget} onCenterChange={setMapCenter}/>
...
function MapController({points,fitRequest,layers,setPois,searchTarget,onCenterChange}:{points:[number,number][];fitRequest:number;layers:string[];setPois:(v:Poi[])=>void;searchTarget:[number,number]|null;onCenterChange:(v:[number,number])=>void}){
  const map=useMap();
  ...
  const requestPois=async(lat:number,lon:number,force=false)=>{
    ...
    try{
      ...
      const data=await res.json();
      if(res.ok){const fresh=(data.pois??[]) as Poi[];writePoiCache(key,fresh);setPois(fresh)}
