...
export function MapView({pins,programItems,members,fitRequest}:Props){
  ...
  function handleMapClick(event:L.LeafletMouseEvent){
    ...
    if(Date.now()-lastMarkerTap.current<350)return;
    setSelected(null);setSelectedTarget(null);setMenuOpen(false);
  }
...
function MapController({points,fitRequest,layers,setPois,searchTarget,selectedTarget,onCenterChange,onMapClick}:{points:[number,number][];fitRequest:number;layers:string[];setPois:(v:Poi[])=>void;searchTarget:[number,number]|null;selectedTarget:[number,number]|null;onCenterChange:(v:[number,number])=>void;onMapClick:(event:L.LeafletMouseEvent)=>void}){
  const map=useMap();
