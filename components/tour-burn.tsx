...
type Props = { mode: "animation" | "final"; onFinished?: () => void; preview?: boolean; onExitPreview?: () => void };

export function TourBurn({ mode, onFinished, preview=false, onExitPreview }: Props) {
  ...
  if(mode==="final") return <div className={styles.finalScreen}>
    <div className={styles.embers}/><div className={styles.finalContent}>
      ...
      {preview&&<><span className={styles.previewBadge}>VORSCHAU – nichts wurde wirklich verbrannt</span>{onExitPreview&&<button type="button" className={styles.exitPreview} onClick={onExitPreview}>Zurück in den Adminmodus</button>}</>}
    </div>
  </div>;

  return <div className={`${styles.overlay} ${styles[`phase${phase}`]}`} aria-live="assertive">
    <div className={styles.flash}/><div className={styles.smoke}/><div className={styles.fireBase}/>
    ...
    <div className={styles.burnCopy}><span>FIRESTARTER 26</span><h1>{phase<2?"Diese Tour wird verbrannt …":"Was auf Tour passiert …"}</h1>{phase>=2&&<p>… bleibt auf Tour.</p>}</div>
  </div>;
}
