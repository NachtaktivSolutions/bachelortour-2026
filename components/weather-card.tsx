...
export function WeatherCard({ latitude, longitude }: { latitude: number; longitude: number }) {
  ...
  useEffect(() => {
    ...
    fetch(url).then(r => r.ok ? r.json() : Promise.reject()).then(json => setData(json.current)).catch(() => setFailed(true));
  }, [latitude, longitude]);
  ...
  return (
    <article className="weather-card">
      ...
      {data && <div className="weather-wind"><Wind size={17}/>{Math.round(data.wind_speed_10m)} km/h</div>}
    </article>
  );
}
