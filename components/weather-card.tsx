...
export function WeatherCard({ latitude, longitude }: { latitude: number; longitude: number }) {
  ...
  return (
    <>
      ...
      {easterOpen&&<div role="dialog" aria-modal="true" aria-label="Wetter Easter Egg" style={{position:"fixed",inset:0,zIndex:10000,display:"flex",alignItems:"center",justifyContent:"center",padding:"18px",background:"rgba(0,0,0,.86)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)"}}>
        <div style={{position:"relative",width:"min(88vw,480px)",maxHeight:"88dvh",borderRadius:"28px",overflow:"hidden",border:"1px solid rgba(255,255,255,.14)",boxShadow:"0 24px 90px rgba(0,0,0,.72)",background:"#111"}}>
          <img src="/weather-easter-egg-photo.svg" alt="Bus an einer roten Ampel auf einer weiten Wiese mit Hasen" style={{display:"block",width:"100%",height:"auto",maxHeight:"88dvh",objectFit:"contain"}}/>
